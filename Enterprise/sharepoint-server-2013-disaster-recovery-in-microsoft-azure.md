...
使用 [SharePoint 2013 的 Microsoft Azure 体系结构](microsoft-azure-architectures-for-sharepoint-2013.md)中的指导设计灾难恢复环境，包括 SharePoint 恢复场。你可以按照 [Azure 中的 SharePoint 灾难恢复解决方案](https://go.microsoft.com/fwlink/p/?LinkId=392554) Visio 文件中的图开始设计过程。我们建议你先设计整个环境，然后开始在 Azure 环境中执行任何工作。
...
## 阶段 3：将 Active Directory 和域名服务部署到 Azure 虚拟网络
<a name="Phase3"> </a>
...
## 阶段 4：在 Azure 中部署 SharePoint 恢复场
<a name="Phase4"> </a>
...
在 SQL Server Management Studio 中，右键单击 **WSS_Content** 数据库，指向" **任务**">" **还原**"，然后单击" **事务日志**"（如果你还没有还原完整备份，则不可用）。有关详细信息，请参阅[还原事务日志备份 (SQL Server)](https://go.microsoft.com/fwlink/p/?LinkId=392778)。
...
在命令行运行 **psconfig.exe** 时，我们使用了 _skipRegisterAsDistributedCachehost_ 参数。有关详细信息，请参阅[在 SharePoint 中规划源和分布式缓存服务 (SharePoint Server 2013)](https://go.microsoft.com/fwlink/p/?linkid=270985)。
...
### 使用术语库管理工具导致了错误："托管元数据存储或连接当前不可用。"
...
### Get-ADForest Windows PowerShell 命令会生成错误："术语'Get-ADForest'未识别为 cmdlet、函数、脚本文件或可运行程序的名称。"

设置用户配置文件时，您需要 Active Directory 林名称。在"添加角色和功能"向导中，确保已启用 Active Directory Module for Windows PowerShell（在"远程服务器管理工具>角色管理工具>AD DS 和 AD LDS 工具"部分）。此外，运行以下命令，然后再使用 **Get-ADForest** ，以确保已加载软件相关项。
...
### 在"<服务器名称>"上启动"AlwaysOn_health"XEvent 会话时可用性组创建失败

确保故障转移群集中的两个节点已启动，没有暂停或停止。
  
### SQL Server 日志传送作业失败，并显示尝试连接到文件共享时出现访问拒绝错误

确保您的 SQL Server Agent 在网络凭据而不是默认凭据下运行。
  
### SQL Server 日志传送作业指示成功，但未复制任何文件

发生这种情况是因为可用性组的默认备份首选项是"首选辅助"。确保您是从可用性组的辅助服务器而不是主服务器运行日志传送作业，否则，作业将失败且无提示。
  
### Managed Metadata Service（或其他 SharePoint 服务）在安装后无法自动启动
...
### 将 DNS 更改为 Azure 故障转移环境之后，客户端浏览器继续使用 SharePoint 网站的旧 IP 地址
...
[SharePoint 数据库的受支持的高可用性和灾难恢复选项 (SharePoint 2013)](https://go.microsoft.com/fwlink/p/?LinkId=393121)
