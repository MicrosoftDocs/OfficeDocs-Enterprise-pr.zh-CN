 ...
 **摘要：** 在 Microsoft Azure 中创建一个简化的内部网作为开发/测试环境。
...
这种配置使得 DC1、APP1、CLIENT 1 以及其他企业网络子网计算机可以： 
...
- 作为自己设计的扩展测试环境的初始配置，包括附加的虚拟机、Azure 服务或其他 Microsoft 云产品，如 Office 365 和企业版安全性 + 移动性 (EMS)。
    
在 Azure 中设置基本配置测试环境有如下四个阶段：
...
若要检查 APP1 和 DC1 之间的名称解析和网络通信，请运行 **ping dc1.corp.contoso.com** 命令，并验证是否存在四个答复。
...
CLIENT1 可充当 Contoso Intranet 中的典型笔记本电脑、平板电脑或台式计算机。

> [!NOTE]  
> 以下命令集创建运行 Windows Server 2016 Datacenter 的 CLIENT1，此操作适用于所有类型的 Azure 订阅。若有基于 Visual Studio 的 Azure 订阅，可以通过 [Azure 门户](https://portal.azure.com)创建运行 Windows 10 的 CLIENT1。 
...
若要检查 CLIENT1 和 DC1 之间的名称解析和网络通信，请运行 Windows PowerShell 命令符中的 **ping dc1.corp.contoso.com** 命令，并验证是否存在四个答复。
...
Azure 中的基本配置现在已完成，可用于应用程序开发和测试或用于构建其他测试环境。 
