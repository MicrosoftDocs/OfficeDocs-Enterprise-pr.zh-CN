...
11. 选择刚刚创建的搜索，然后单击 "**刷新** ![ 刷新" ](media/165fb3ad-38a8-4dd9-9e76-296aefd96334.png) 以更新详细信息窗格中显示的信息。 "**估计成功**状态" 指示搜索已完成。 Exchange 还显示根据您在步骤9中指定的搜索条件，搜索找到的项目总数（及其大小）的估计值。 
...
如果无法通过预览搜索结果找到项目，或者如果要查看用户的 "可恢复的项目" 文件夹中的项目，则可以将搜索结果复制到一个特殊的邮箱（称为发现邮箱），然后在 web 上的 Outlook 中打开该邮箱以查看实际项目。 复制搜索结果的最佳原因是，您可以查看用户的 "可恢复的项目" 文件夹中的项目。 您尝试恢复的项目很可能位于 "清除" 子文件夹中。 
