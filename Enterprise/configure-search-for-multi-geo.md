...
只要多地理环境具有设置，用户将看到新闻，最近和已跟随从其 SharePoint 主页上的多个地理位置的站点。如果他们在 SharePoint 的主页上使用的搜索框，它们将从多个地理位置获得合并的结果。
...
1. 搜索客户端调用搜索 REST 端点查询属性 EnableMultiGeoSearch = true。
2. 查询被发送到组织中的所有地理位置。
3. 从每个地理位置的搜索结果将合并和排名。
4. 客户端获取统一的搜索结果。



<span id="_Set_up_a" class="anchor"><span id="_Ref501388384" class="anchor"></span></span>请注意我们不合并搜索结果，直到我们已收到来自所有地理位置的结果。这意味着多地区搜索有额外延迟相比于只有一个地理位置与环境中的搜索。

<span id="_Set_up_a_1" class="anchor"><span id="_Ref505252370" class="anchor"></span></span>
## <a name="get-a-search-center-to-show-results-from-all-geo-locations"></a>获取搜索中心显示来自所有地理位置的结果
...
3.  选择要设置，单击右上角，**设置**齿轮图标，然后单击**编辑页面**的垂直。在编辑模式下打开搜索结果页。

     ![](media/configure-search-for-multi-geo_image2.png)
1.  在搜索结果 Web 部件中，将鼠标指针移动到上面，右上角的 Web 部件中，单击箭头，然后单击在菜单上的**编辑 Web 部件**。在功能区中打开搜索结果 Web 部件工具窗格页面右上角。![](media/configure-search-for-multi-geo_image3.png)
...
MultiGeoSearchStatus – 这是 SharePoint 搜索 API 对请求的响应中返回的属性。属性的值是一个字符串，并给出了 SharePoint 搜索 API 返回的结果有关的下列信息：
