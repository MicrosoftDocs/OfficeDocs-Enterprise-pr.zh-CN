...
通用配置是允许, 而无需处理来自代理服务器的所有出站通信, 以获取命中代理服务器的 Office 365 网络流量的目标 IP 地址。有关与 SSL 中断和检查有关的问题的信息, 请参阅[使用第三方网络设备或解决方案在 Office 365 流量](https://support.microsoft.com/en-us/help/2690045/using-third-party-network-devices-or-solutions-with-office-365)中。
...
有关 Microsoft 流示例和模板的信息, 请参阅[使用 microsoft 流接收对 Office 365 IP 地址和 url 所做更改的电子邮件](https://techcommunity.microsoft.com/t5/Office-365-Networking/Use-Microsoft-Flow-to-receive-an-email-for-changes-to-Office-365/td-p/240651)
 ...
 **租户位置**最好是使用我们的[数据中心地图](http://aka.ms/datamaps)来确定的。
...
1. 使用[CIDR 计算器](http://jodies.de/ipcalc)检查 IP 地址是否包含在更大的已发布区域中。
...
office 365 和其他 Microsoft 服务使用几种第三方服务 (如 Akamai 和 MarkMonitor) 来改进 Office 365 体验。为了让你能够获得最佳体验, 我们可能会在将来更改这些服务。执行此操作时, 我们通常会发布指向第三方拥有的域、记录或 IP 地址的 CNAME 记录。第三方域可以承载内容 (如 CDN), 也可以托管服务, 例如地理流量管理服务。当您看到与这些第三方的连接时, 它们的形式是重定向或引用, 而不是来自客户端的初始请求。有些客户需要确保这种形式的引用和重定向可以通过, 而无需明确添加可能使用的潜在 fqdn 的长列表。
  
服务列表可能随时更改。当前使用的某些服务包括:
...
Office 365 是一套在 internet 上运行的服务, 可靠性和可用性承诺基于许多可用的标准 internet 服务。例如, 诸如 DNS、CRL 和 cdn 等标准 internet 服务必须能够使用 Office 365, 就像它们必须可访问的那样, 才能使用最新式的 internet 服务。
...
|**共** <br/> |office 365 Pro Plus、office Online、Azure AD 和其他常见网络终结点 <br/> |
...
限制对使用者服务的访问权限应由您自己承担, 阻止使用者服务的唯一可靠方法是限制对*login.live.com* FQDN 的访问。此 FQDN 由广泛的一组服务使用, 包括非消费者服务 (如 MSDN、TechNet 和其他服务)。此 FQDN 也由 Microsoft 支持的安全文件交换程序使用, 并且必须转移文件以促进 Microsoft 产品的故障排除。 限制对此 FQDN 的访问可能会导致需要为与这些服务关联的网络请求包含规则例外。
